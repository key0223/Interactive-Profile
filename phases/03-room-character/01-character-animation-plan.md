...
- Related Scripts: `Assets/02.Scripts/Core/Player/PlayerMovement.cs`, `Assets/02.Scripts/Core/Player/PlayerSpriteAnimator.cs`, `Assets/02.Scripts/Core/Cat/CatSpriteAnimator.cs`, `Assets/02.Scripts/Core/Cat/CatAmbientAI.cs`, `Assets/02.Scripts/Core/Input/InputManager.cs`
...
고양이 ambient animation은 Player 시스템에 섞지 않고 별도 Cat 시스템으로 관리한다. Cat도 Animator Controller 없이 코드 기반 `SpriteRenderer` 교체 방식을 사용한다.
...
- Cat은 `CatSpriteAnimator`와 `CatAmbientAI`로 분리하며, Player 입력이나 Player 위치를 참조하지 않는다.
...
## CatSpriteAnimator And Ambient AI Policy

Cat animation은 Player animation과 같은 단일 `SpriteRenderer.sprite` 교체 정책을 따른다.

구성:

- `CatSpriteAnimator`는 `Idle`, `Walk`, `Sleep` 상태를 가진다.
- 방향은 `Down`, `Left`, `Right`, `Up`이다.
- 방향별 Sprite 배열을 사용하고, 상태 또는 방향 변경 시 frame index를 초기화한다.
- 특정 방향 배열이 비어 있으면 같은 상태의 Down 또는 첫 유효 방향 배열로 fallback 한다.
- 모든 배열이 비어 있으면 exception 없이 현재 sprite를 유지한다.

`CatAmbientAI`는 Player를 따라다니지 않는 독립 상태머신이다.

- 일정 시간 idle 후 wander 또는 sleep 루틴을 선택한다.
- wander point가 있으면 그중 랜덤 목적지로 이동한다.
- wander point가 없으면 현재 위치 주변 작은 랜덤 offset으로 이동한다.
- sleep spot이 있으면 해당 위치까지 이동한 뒤 sleep 한다.
- sleep spot이 없으면 현재 위치에서 sleep 한다.
- 이동 중 방향은 이동 벡터 기준으로 갱신한다.
- NavMesh, pathfinding, 공격, 추적, 펫 follow 로직은 범위 밖이다.
...
- Cat sprite도 같은 기준으로 Editor에서 slice하고 `CatSpriteAnimator` 배열에 직접 입력한다.
...
- `CatAmbientAI`가 Player 위치, Player 입력, Computer UI 상태를 참조하지 않는다.
- Cat ambient 이동에 NavMesh 또는 복잡한 pathfinding을 추가하지 않는다.
...
Cat 작업은 [Ambient Cat AI Guide](./03-ambient-cat-ai-guide.md)에 따라 별도 수행한다.
