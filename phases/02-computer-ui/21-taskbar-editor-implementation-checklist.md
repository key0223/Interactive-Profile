...
- `TaskbarRoot`와 `TaskbarButtonRoot`가 준비되어 있다.
- `ProjectTaskbarUI._buttonRoot`가 연결되어 있다.
- `ProjectTaskbarUI._buttonPrefab`이 연결되어 있다.
- `ProjectTaskbarButtonUI._button`과 `_titleText`가 연결되어 있다.
- `ProjectDesktopUI._projectTaskbarUI`가 연결되어 있다.
- `WindowLayer Bottom`이 `TaskbarRoot Height`와 맞는다.
- 서로 다른 프로젝트 2개 open 시 runtime taskbar button 2개가 생성된다.
- 같은 프로젝트 재오픈 시 기존 button/window가 재사용된다.
- minimize/restore/focus/close/maximize bounds가 검증되었다.

## Completed Step Summary

아직 실행 전이다. 완료 시 실제 Unity Editor hierarchy, Inspector 연결, `WindowLayer` bounds 조정, Play Mode 검증 결과를 기록한다.
