...
현재 구현 기준으로 folder 선택은 message list 필터로 동작한다. `Inbox`는 전체 메시지를 표시하고, `GitHub`, `Email`, `Portfolio`, `Resume`은 해당 folder entry만 표시한다.
...
- folder row 클릭 시 message list를 필터링하고 선택 folder highlight를 갱신한다.
...
4. 첫 folder는 `Inbox`로 선택하고 전체 entry를 표시한다.
5. folder row 선택 시 message list를 해당 folder 기준으로 필터링한다.
6. 첫 filtered entry를 자동 선택하고 preview pane을 채운다.
7. message row 선택 시 preview pane의 subject, description, url/status가 갱신된다.
8. `CONNECT` 버튼은 선택된 entry의 `url`을 `Application.OpenURL`로 연다.

row 구현 기준:

- message list는 `ContactMessageRowUI` prefab 기반으로 구성한다.
- folder list는 `ContactFolderRowUI` prefab 기반으로 구성한다.
- TMP-only message list는 legacy fallback으로만 유지한다.
...
{
    public ContactFolderType folderType;
...
- `folderType`: `Inbox`, `GitHub`, `Email`, `Portfolio`, `Resume` 중 하나. `Inbox`는 전체 보기로 취급한다.
