...
현재 foundation은 단일 탑다운 방 탐색, 오브젝트 상호작용, 컴퓨터 UI 진입, runtime desktop icon, project window, README.TXT, SYSTEM.LOG, CONTACT.EXE, taskbar/window lifecycle을 우선한다. 새 기능은 특정 오브젝트나 프로젝트 하나에 고정하지 않고, Inspector 참조와 데이터 추가로 확장 가능한 낮은 결합도 구조를 유지한다.
...
**결정**: 현재 architecture foundation은 방 탐색, 상호작용, 컴퓨터 UI, runtime desktop app 구성, project/typed window lifecycle까지 확정한다.

**이유**:
- PRD의 제외 범위는 전투, 퀘스트, 인벤토리, 저장/로드, 네트워크, 다중 방, 복잡한 대화 시스템을 명시한다.
- 범용 게임 프레임워크를 먼저 만들면 구현 비용이 증가하고 실제 포트폴리오 경험 검증이 늦어진다.

**트레이드오프**:
- 나중에 다중 화면, 대화 시스템, 저장/로드가 필요해지면 추가 ADR로 구조를 확장해야 한다.
...
- `README.TXT`, `SYSTEM.LOG`, `CONTACT.EXE` typed app window는 `DesktopWindowId.ForType(type)` 경로로 관리한다.
